...
water_samples_feb <- read_csv("water_samples_feb2022.csv")

# two samples per site, syringe and a whirlpack (WP), matrix also came from whirlpacks
...
glow_water_samples <- read_csv("water_samples_feb2022_fluorometry.csv") %>%
  mutate(mean_FLU = rowMeans(cbind(FLU1, FLU2, FLU3)))

# load standard curve data
standard_water_samples <- read_csv("water_samples_feb2022_standard.csv") %>%
  mutate(mean_FLU = rowMeans(cbind(FLU1, FLU2, FLU3)))
...
water_samples_feb2 <- water_samples_feb %>%
...
water_samples_feb_spike <- water_samples_feb2 %>%
  filter(sample == "matrix") %>%
  transmute(site_ID = site_ID,
            Fsm_spike = mean_FLU)

water_samples_feb_zero <- water_samples_feb2 %>%
  filter(syringe == "whirlpack") %>%
  filter(sample != "matrix")
...
water_samples_feb_matrix <- water_samples_feb_zero %>% 
  left_join(water_samples_feb_spike, by = "site_ID") %>%
  mutate(
...
water_samples_feb3 <- water_samples_feb2 %>%
  left_join(water_samples_feb_matrix, by = "site_ID") %>%
  mutate(
